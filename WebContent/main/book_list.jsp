...
<body>
								...
								<div>
									<div class="second_fenlei">&middot;
										...
										<a href="${pageContext.request.contextPath}/ProductController/product/selectProductAndCategory.do?id=${c1.dd_category1_id}">
											<c:choose>
												<c:when test="${chooseId == c1.dd_category1_id }">
													<!-- 名称 -->
													<strong style="color:red">${c1.dd_category1_name }</strong>
												
													<!-- 数量 -->
													(${c1.dd_Category1Count })
												</c:when>
												<c:otherwise>
													<span id="">${c1.dd_category1_name }</span>
													  (${c1.dd_Category1Count })
												</c:otherwise>
											</c:choose>
										</a>
									...
									<ul class="ul_left_list">
										<c:forEach var='c2'  items="${c1.dangdang_category2 }">
											<a href="${pageContext.request.contextPath}/ProductController/product/selectProductAndCategory.do?id=${c2.dd_category2_id}">
												<c:choose>
													<c:when test="${chooseId == c2.dd_category2_id }">
														<strong style="color:red">${c2.dd_category2_name }</strong>
														<!-- 数量 -->
														 (${c2.dd_Category2Count }) 
													</c:when>
													<c:otherwise>
														<span id="">${c2.dd_category2_name} </span>
														(${c2.dd_Category2Count })
													</c:otherwise>
												</c:choose>
											</a><br/>
		...
		<div class="book_center">
			...
			<div id="divRight" class="list_right">
				...
				<div id="book_list" class="list_r_title">
					...
					<form action="${pageContext.request.contextPath}/ProductController/product/productsearch.do" method="post">
					图书名称：<input type="text" name="dd_product_name" id="dd_product_name" />
					作者：<input type="text" name="dd_product_author" id="dd_product_author" />
					<!-- 出版社：<input type="text" name="dd_product_press" id="dd_product_press" > -->
				        出版社： <input type="text" name="dd_product_press" id="dd_product_press" list="sexlist">
				            <datalist id="sexlist">
				           <c:forEach var='LP_P' items="${requestScope.ListPro_Press }"> 
				                <option>${LP_P}</option>
				...
				<div class="clear"></div>
					<c:forEach var='s' items="${requestScope.productList }">
						...
						<div class="list_r_list">
								...
								<img src="${pageContext.request.contextPath}/productImages/${s.dd_product_img }" border="0" />
								</a>
								</span>
								<h2>
									<a href="${pageContext.request.contextPath }/ProductController/product/productshow.do?ddProId=${s.dd_product_id}" target='_blank'> 
									 书名：${s.dd_product_name }
									 </a>
								</h2>
								<h3>
									顾客评分：
								...
								<h5>
									内容简介：${s.dd_product_brief }
								</h5>
								<div class="clear"></div>
								<h6>
									<span class="del">定价：￥${s.dd_product_price }&nbsp;&nbsp;
									</span> <span class="red">当当价：￥${s.dd_product_ddprice }&nbsp;&nbsp;
									</span> 节省：￥${s.dd_product_price -  s.dd_product_ddprice}
								</h6> 
								
								<span class="list_r_list_button">
								<a href="javascript:void(0)">
										<img src='${pageContext.request.contextPath}/images/buttom_goumai.gif' class="abc"
										id="buy${s.dd_product_id }"
										onclick="addCart('${s.dd_product_id }')" />
								</a> 
	...
	<script type="text/javascript">
	function searchBook(){
		var ddproductname = $("#dd_product_name").val();
		var ddproductauthor = $("#dd_product_author").val();
		var ddproductpress = $("#dd_product_press").val();
		if(ddproductpress == 0){
			ddproductpress = "";
		}
		window.location.href="${pageContext.request.contextPath}/ProductController/product/selectProductAndCategory.do?ddproductname="+ddproductname+"&ddproductauthor="+ddproductauthor+"&ddproductpress="+ddproductpress+"&id=0";
	}
	</script>
</body>
</html>
